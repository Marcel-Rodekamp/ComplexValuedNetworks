...
LOSSES = torch.zeros(NLF,NAF,NLCN, NLR, NPR, NINT,EPOCHS)
LossValid = torch.zeros(NLF,NAF,NLCN, NLR,NPR, NINT,EPOCHS)
BigLoss = torch.zeros(NLF,NAF,NLCN, NLR, NPR, NINT)
                    ...
                    for m in range(NINT):   # number of internal layers
                        ...
                        for epoch in range(EPOCHS):
                            ...
                            with torch.no_grad():
                                ...
                                LossValid[p][n][i][j][k][m][epoch] = Loss(PHIvalid, logD)

                        BigLoss[p][n][i][j][k][m] = loss.item()

                        #plotting

                        fig = plt.figure(figsize=(7,5))
                        plt.plot(epochs, LOSSES[p][n][i][j][k][m].detach().numpy(),  '-r', label='Test loss function')
                        plt.yscale('log')
                        plt.xlabel('Epoch')
                        plt.ylabel('Log of loss function')

                        plt.plot(epochs, LossValid[p][n][i][j][k][m].detach().numpy(), '-b', label='Validation loss function')
                        plt.legend()
