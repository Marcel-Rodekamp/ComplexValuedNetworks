...
def pathnameh(hyperparameters):
    
    if 'epoch' in hyperparameters:
        Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate, epoch = decodeHyperparams(hyperparameters)
        return f",Nt_{Nt}initEpsilon_{initEpsilon},trainEpsilon_{trainEpsilon},trainBatchSize_{trainBatchSize},trainMiniBatchSize_{trainMiniBatchSize},learningRate_{learningRate}, numPRCLLayers_{numPRCLLayers},numInternalLayers_{numInternalLayers}, activation_{activation}, lossfn_{lossFct}, epochs_{epoch}.h5"
    else:
        Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate = decodeHyperparams(hyperparameters)
        return f",Nt_{Nt}initEpsilon_{initEpsilon},trainEpsilon_{trainEpsilon},trainBatchSize_{trainBatchSize},trainMiniBatchSize_{trainMiniBatchSize},learningRate_{learningRate}, numPRCLLayers_{numPRCLLayers},numInternalLayers_{numInternalLayers}, activation_{activation}, lossfn_{lossFct}.h5"
...
def plot_action_evolution(S_full, S, burnIn, Nconf, hyperparameters):
    plt.plot(np.arange(0,burnIn+Nconf), S_full.detach().real.numpy(), 'x', label = r"$S[\phi_n]$")
    
    S_est = np.ones(burnIn+Nconf)*S.real.mean().item()
    S_err = np.ones(burnIn+Nconf)*S.real.std().item()
    ...
    plt.legend()
    plt.title(f"Action Evolution,burnIn={burnIn}{plotTitle(hyperparameters)}")
    plt.savefig(f"Action Evolution,burnIn={burnIn}{pathname(hyperparameters)}",bbox_inches='tight', dpi=150)
    plt.close() 
    
    
def plot_stat_power(x,result, error, hyperparameters):
    fig, ax = plt.subplots(1, 1, figsize=(7, 5))
    ax.errorbar(x, result, yerr = error, capsize = 2)
    
    ax.set_xlabel("N")
    ax.set_ylabel("Statistical power")
    ax.set_yscale('log')
    
    ax.set_title(f"Statistical Power computed up to different numbers of configurations\n{plotTitle(hyperparameters)}")
    plt.savefig(f"StatPower{pathname(hyperparameters)}",bbox_inches='tight', dpi=150)
    plt.close() 
    
def plot_correlators(t,Cxy_est, Cxy_err, hyperparameters):
    fig, ax = plt.subplots(1, 1, figsize=(7, 5))
    ax.errorbar(t, Cxy_est[:,0,0], yerr = Cxy_err[:,0,0], color = 'r', capsize = 2, label = 'xy = 00')
    ax.errorbar(t, Cxy_est[:,1,1], yerr = Cxy_err[:,1,1], color = 'b', capsize = 2, label = 'xy = 11')
    
    ax.set_xlabel("t")
    ax.set_ylabel("$C_{xy}(t)$")
    ax.set_yscale('log')
    ax.legend()
    ax.set_title(f"Correlator plot\n{plotTitle(hyperparameters)}")
    
    plt.savefig(f"Correl{pathname(hyperparameters)}",bbox_inches='tight', dpi=150)
    plt.close()
