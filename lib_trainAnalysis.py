...
def decodeHyperparams(hyperparameters):
    ...
    if 'epoch' in hyperparameters:
        ...
        return Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate, epoch, okhandle
    else:
        return Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate, okhandle
    
    
def plotTitle(hyperparameters):
    if 'epoch' in hyperparameters:
        Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate, epoch, _ = decodeHyperparams(hyperparameters)
        ...
        Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate, _ = decodeHyperparams(hyperparameters)
        return  f"Nt: {Nt}\ninitEpsilon: {initEpsilon}\ntrainEpsilon: {trainEpsilon}\ntrainBatchSize: {trainBatchSize}\ntrainMiniBatchSize: {trainMiniBatchSize}\nlearningRate: {learningRate}\nnumPRCLLayers: {numPRCLLayers}\nnumInternalLayers: {numInternalLayers}\nactivation: {activation}\nlossfn: {lossFct}"


def pathname(hyperparameters, plot = True):
    
    if plot == True:
    
        if 'epoch' in hyperparameters:
            Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate, epoch, okhandle = decodeHyperparams(hyperparameters)
            return f",Nt_{Nt}initEpsilon_{initEpsilon},trainEpsilon_{trainEpsilon},trainBatchSize_{trainBatchSize},trainMiniBatchSize_{trainMiniBatchSize},learningRate_{learningRate}, numPRCLLayers_{numPRCLLayers},numInternalLayers_{numInternalLayers},activation_{activation},lossfn_{lossFct},epochs_{epoch}"
        else:
            ...
            return f",Nt_{Nt}initEpsilon_{initEpsilon},trainEpsilon_{trainEpsilon},trainBatchSize_{trainBatchSize},trainMiniBatchSize_{trainMiniBatchSize},learningRate_{learningRate}, numPRCLLayers_{numPRCLLayers},numInternalLayers_{numInternalLayers}, activation_{activation}, lossfn_{lossFct}"
    
    else:
        
        if 'epoch' in hyperparameters:
            Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate, epoch, okhandle = decodeHyperparams(hyperparameters)
            return f",Nt_{Nt}initEpsilon_{initEpsilon},trainEpsilon_{trainEpsilon},trainBatchSize_{trainBatchSize},trainMiniBatchSize_{trainMiniBatchSize},learningRate_{learningRate}, numPRCLLayers_{numPRCLLayers},numInternalLayers_{numInternalLayers},activation_{activation},lossfn_{lossFct},epochs_{epoch},{okhandle}"
        else:
            Nt, numPRCLLayers, numInternalLayers, activation, initEpsilon, trainEpsilon, trainBatchSize, trainMiniBatchSize, lossFct, learningRate = decodeHyperparams(hyperparameters)
            return f",Nt_{Nt}initEpsilon_{initEpsilon},trainEpsilon_{trainEpsilon},trainBatchSize_{trainBatchSize},trainMiniBatchSize_{trainMiniBatchSize},learningRate_{learningRate}, numPRCLLayers_{numPRCLLayers},numInternalLayers_{numInternalLayers}, activation_{activation}, lossfn_{lossFct},{okhandle}"
...
def plot_loss_eq2(EPOCHS,lossTrainEq, lossTrainNoEq, lossValidEq, lossValidNoEq,lossTrainEqErr, lossTrainNoEqErr, lossValidEqErr, lossValidNoEqErr, hyperparameters):
    ...
    fig.tight_layout()
    fig.suptitle(f"Loss fn plots\n{plotTitle(hyperparameters)}",y=1.4)
    fig.savefig(f"Lossfn_plots{pathname(hyperparameters)}.pdf",bbox_inches='tight', dpi=150)
    fig.clear()
    plt.close(fig) 
...
def plot_actionStatistics2(validDataEq, validDataNoEq, NNvalidDataEq, NNvalidDataNoEq, logDetValidEq, logDetValidNoEq, hyperparameters, HM):
    ...
    S_NNvalidDataNoEq, S_validDataNoEq, diffSvalidNoEq, y1phiNoEq, y1NNphiNoEq, x1phiNoEq, x1NNphiNoEq, x2phiNoEq, x2NNphiNoEq, x3phiNoEq,\
                        x3NNphiNoEq, x4phiNoEq, x4NNphiNoEq, y3phiNoEq, y2phiNoEq, y2NNphiNoEq, y4phiNoEq = get_x_y_values(validDataNoEq, NNvalidDataNoEq, logDetValidNoEq, HM)
    ...
    outer = gridspec.GridSpec(7, 2, wspace=0.2, hspace=0.2,height_ratios=[1,1,1,2,2,2,2])
    
    
    
    plt.figtext(0.25, 0.90, "Equivariance", fontsize = 'x-large')
    plt.figtext(0.70, 0.90, "No Equivariance", fontsize = 'x-large')
    ...
    # real part of S should increase - or not
    ax0.plot(S_NNvalidDataNoEq.real.detach().numpy())
    ax0.set_xlabel('n')
    ax0.set_ylabel('Re(S(NNphi))')
    ax0.set_title('Re(S(NNphi))')
    ...
    ax1.plot(S_NNvalidDataNoEq.imag.detach().numpy())
    ax1.set_xlabel('n')
    ax1.set_ylabel('Im(SNN(phi))')
    ax1.set_title('Im(SNN(phi))')
    ...
    # subplot: i=3. Safety checks
    # no equivariance
    
    # Add the two sub-subplots on the left and right
    
    inner = gridspec.GridSpecFromSubplotSpec(1, 2,subplot_spec=outer[3], wspace=W, hspace=H)
    ax0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(ax0)
    ...
    ax0.plot(logDetValidNoEq.real.detach().numpy())
    ax0.set_xlabel('n')
    ax0.set_ylabel('Re(log det J)')
    ax0.set_title('Re(log det J)')
    ...
    ax1.plot(logDetValidNoEq.imag.detach().numpy())
    ax1.set_xlabel('n')
    ax1.set_ylabel('Im(log det J)')
    ax1.set_title('Im(log det J)')
    ...
    inner = gridspec.GridSpecFromSubplotSpec(1, 2,subplot_spec=outer[4], wspace=W, hspace=H)
    ax0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(ax0)
    ...
    # subplot: i=5. Safety checks
    # no equivariance
    
    # Add the two sub-subplots on the left and right
    
    inner = gridspec.GridSpecFromSubplotSpec(1, 2,subplot_spec=outer[5], wspace=W, hspace=H)
    ax0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(ax0)
    ...
    # real part of S should increase - or not
    ax0.plot(SeffNoEq.real.detach().numpy())
    ax0.set_xlabel('n')
    ax0.set_ylabel('Re(Seff(phi))')
    ax0.set_title('Re(Seff(phi))')
    ...
    ax1.plot(SeffNoEq.imag.detach().numpy())
    ax1.set_xlabel('n')
    ax1.set_ylabel('Im(Seff(phi))')
    ax1.set_title('Im(Seff(phi))')
    ...
    inner = gridspec.GridSpecFromSubplotSpec(2, 2,subplot_spec=outer[7], wspace=W, hspace=H)
    axs0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(axs0)
    ...
    axs0.plot(x1phiNoEq, y1phiNoEq, '.r', label = 'phi')
    axs0.plot(x1NNphiNoEq, y1NNphiNoEq, '.b', label = 'NN(phi)')
    axs0.set_xlabel('Re <phi>')
    axs0.set_ylabel('Re S')
    axs0.set_title('Re S for phi and NN(phi)')


    axs1.plot(x2phiNoEq,y1phiNoEq, '.r', label = 'phi')
    axs1.plot(x2NNphiNoEq, y1NNphiNoEq, '.b', label = 'NN(phi)')
    axs1.set_xlabel('Im <phi>')
    axs1.set_ylabel('Re S')
    axs1.set_title('Re S for phi and NN(phi)')


    axs2.plot(x3phiNoEq, y1phiNoEq, '.r', label = 'phi')
    axs2.plot(x3NNphiNoEq, y1NNphiNoEq, '.b', label = 'NN(phi)')
    axs2.set_xlabel('|Re phi|')
    axs2.set_ylabel('Re S')
    axs2.set_title('Re S for phi and NN(phi)')

    axs3.plot(x4phiNoEq, y1phiNoEq, '.r', label = 'phi')
    axs3.plot(x4NNphiNoEq, y1NNphiNoEq, '.b', label = 'NN(phi)')
    axs3.set_xlabel('|Im phi|')
    axs3.set_ylabel('Re S')
    axs3.set_title('Re S for phi and NN(phi)')
    ...
    inner = gridspec.GridSpecFromSubplotSpec(2, 2,subplot_spec=outer[8], wspace=W, hspace=H)
    axs0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(axs0)
    ...
    # subplot: i=7. y = Re Seff
    # no equivariance 
    # add the four sub-subplots   
        
    inner = gridspec.GridSpecFromSubplotSpec(2, 2,subplot_spec=outer[9], wspace=W, hspace=H)
    axs0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(axs0)
    ...
    axs0.plot(x1phiNoEq, y3phiNoEq, '.r', label = 'Seff(phi)')
    axs0.plot(x1phiNoEq, y1phiNoEq, '.b', label = 'S(phi)')
    axs0.set_xlabel('Re <phi>')
    axs0.set_ylabel('Re S or Re Seff')
    axs0.set_title('Re S(phi) vs Seef(phi)')

    axs1.plot(x2phiNoEq,y3phiNoEq, '.r', label = 'Seff(phi)')
    axs1.plot(x2phiNoEq,y1phiNoEq, '.b', label = 'S(phi)')
    axs1.set_xlabel('Im <phi>')
    axs1.set_ylabel('Re S or Re Seff')
    axs1.set_title('Re S(phi) vs Seef(phi)')

    axs2.plot(x3phiNoEq, y3phiNoEq, '.r', label = 'Seff(phi)')
    axs2.plot(x3phiNoEq, y1phiNoEq, '.b', label = 'S(phi)')
    axs2.set_xlabel('|Re phi|')
    axs2.set_ylabel('Re S or Re Seff')
    axs2.set_title('Re S(phi) vs Seef(phi)')

    axs3.plot(x4phiNoEq, y3phiNoEq, '.r', label = 'Seff(phi)')
    axs3.plot(x4phiNoEq, y1phiNoEq, '.b', label = 'S(phi)')
    axs3.set_xlabel('|Im phi|')
    axs3.set_ylabel('Re S or Re Seff')
    axs3.set_title('Re S(phi) vs Seef(phi)') 
    ...
    inner = gridspec.GridSpecFromSubplotSpec(2, 2,subplot_spec=outer[10], wspace=W, hspace=H)
    axs0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(axs0)
    ...
    # subplot: i=9. Im S mod 2pi
    
    # create the 4 subplots

    inner = gridspec.GridSpecFromSubplotSpec(2, 2,subplot_spec=outer[11], wspace=W, hspace=H)
    axs0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(axs0)
    ...
    axs0.plot(x1phiNoEq, y2phiNoEq, '.r', label = 'phi')
    axs0.plot(x1NNphiNoEq, y2NNphiNoEq, '.b', label = 'NN(phi)')
    axs0.set_xlabel('Re <phi>')
    axs0.set_ylabel('Im S mod 2π')
    axs0.set_title('Im S mod 2π for phi and NN(phi)')

    axs1.plot(x2phiNoEq,y2phiNoEq, '.r', label = 'phi')
    axs1.plot(x2NNphiNoEq, y2NNphiNoEq, '.b', label = 'NN(phi)')
    axs1.set_xlabel('Im <phi>')
    axs1.set_ylabel('Im S mod 2π')
    axs1.set_title('Im S mod 2π for phi and NN(phi)')

    axs2.plot(x3phiNoEq, y2phiNoEq, '.r', label = 'phi')
    axs2.plot(x3NNphiNoEq, y2NNphiNoEq, '.b', label = 'NN(phi)')
    axs2.set_xlabel('|Re phi|')
    axs2.set_ylabel('Im S mod 2π')
    axs2.set_title('Im S mod 2π for phi and NN(phi)')

    axs3.plot(x4phiNoEq, y2phiNoEq, '.r', label = 'phi')
    axs3.plot(x4NNphiNoEq, y2NNphiNoEq, '.b', label = 'NN(phi)')
    axs3.set_xlabel('|Im phi|')
    axs3.set_ylabel('Im S mod 2π')
    axs3.set_title('Im S mod 2π for phi and NN(phi)')
    ...
    inner = gridspec.GridSpecFromSubplotSpec(2, 2,subplot_spec=outer[12], wspace=W, hspace=H)
    axs0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(axs0)
    ...
    # subplot: i=11. y = Im Seff mod 2 pi or Im S mod 2pi
    # Seff
    
    # create the 4 subplots

    inner = gridspec.GridSpecFromSubplotSpec(2, 2,subplot_spec=outer[13], wspace=W, hspace=H)
    axs0 = plt.Subplot(fig, inner[0])
    fig.add_subplot(axs0)
    ...
    axs0.plot(x1phiNoEq, y4phiNoEq, '.r', label = 'Seff(phi)')
    axs0.plot(x1phiNoEq, y2phiNoEq, '.b', label = 'S(phi)')
    axs0.set_xlabel('Re <phi>')
    axs0.set_ylabel('Im Seff or S vs mod 2π')
    axs0.set_title('Im S mod 2π vs Im Seff mod 2π')

    axs1.plot(x2phiNoEq,y4phiNoEq, '.r', label = 'Seff(phi)')
    axs1.plot(x2phiNoEq,y2phiNoEq, '.b', label = 'S(phi)')
    axs1.set_xlabel('Im <phi>')
    axs1.set_ylabel('Im Seff or S mod 2π')
    axs1.set_title('Im S mod 2π vs Im Seff mod 2π')

    axs2.plot(x3phiNoEq, y4phiNoEq, '.r', label = 'Seff(phi)')
    axs2.plot(x3phiNoEq, y2phiNoEq, '.b', label = 'S(phi)')
    axs2.set_xlabel('|Re phi|')
    axs2.set_ylabel('Im Seff or S mod 2π')
    axs2.set_title('Im S mod 2π vs Im Seff mod 2π')

    axs3.plot(x4phiNoEq, y4phiNoEq, '.r', label = 'Seff(phi)')
    axs3.plot(x4phiNoEq, y2phiNoEq, '.b', label = 'S(phi)')
    axs3.set_xlabel('|Im phi|')
    axs3.set_ylabel('Im Seff or S mod 2π')
    axs3.set_title('Im S mod 2π vs Im Seff mod 2π')
    ...
    fig.savefig(f"Sstat{pathname(hyperparameters)}.pdf",bbox_inches='tight', dpi=150)
    fig.clear()
    plt.close(fig) 
    
    

def plot_fieldStatistics2(validDataEq, validDataNoEq, NNvalidDataEq, NNvalidDataNoEq, logDetValidEq, logDetValidNoEq, hyperparameters, HM):
    ...
    fig.savefig(f"Fieldstat{pathname(hyperparameters)}.pdf",bbox_inches='tight', dpi=150)
    fig.clear()
    plt.close(fig) 
