...
class Hubbard2SiteModel:
    def __init__(self,Nt,beta,U,mu, tangentPlaneOffset = 0):
        ...
        # exp( \kappa + C^p )
        self.expKappa_p = torch.zeros((self.Nx, self.Nx),dtype = torch.cdouble)
        ...
        # exp( \kappa + C^h )
        self.expKappa_h = torch.zeros((self.Nx, self.Nx),dtype = torch.cdouble)
    ...
    def M(self,phi,species):
        ...
        M = torch.zeros((self.Nt, self.Nx, self.Nt, self.Nx), dtype = torch.cdouble)
        ...
        M[ts,:,ts,:] = torch.eye(self.Nx,dtype = torch.cdouble)
        M[self.Nt - 1, :, self.Nt - 1, :] = torch.eye(self.Nx,dtype = torch.cdouble)
    ...
    def TrMinvM(self,phi,species):
        ...
        Tr = torch.zeros((self.Nt,self.Nx),dtype = torch.cdouble)
